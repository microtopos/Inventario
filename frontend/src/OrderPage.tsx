...
import { confirmDraft } from "./orderService"
...
export default function OrderPage({ onNavigate }: {
  ...
  useEffect(() => {
    getProductsWithSizes().then(prods => {
      setProducts(prods)
      setReady(true)
    })
  }, [])
  ...
  function setNotas(notas: string) {
    setDraft(draftItems, notas)
  }

  function totalPedido() {
    return Object.values(draftItems).reduce((s, v) => s + (Number(v) || 0), 0)
  }

  function construirPedido() {
    const map: Record<number, { producto: any; tallas: any[]; total: number }> = {}
    for (const tallaId in draftItems) {
      const cantidad = draftItems[Number(tallaId)]
  ...
  async function handleDiscard() {
    const ok = await confirm(
      "¿Descartar el borrador? Se perderán todos los cambios.",
  ...
  async function confirmOnly() {
    ...
    try {
      ...
      const confirmedId = await confirmDraft(id, draftNotas)
      clearState()
  ...
  async function exportPDF() {
    try {
      ...
      await confirmDraft(id, draftNotas)
      clearState()
      toast.success("PDF guardado", filePath)
      backupDBSilent().catch(() => {})
      setSelectedProduct(null)
    } catch (e: any) {
      if (e?.message === "Selección cancelada") return
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f0f2f5", fontFamily: "system-ui, sans-serif" }}>
      ...
      <main style={{ maxWidth: "1200px", margin: "0 auto", padding: "24px", display: "grid", gridTemplateColumns: "260px 1fr 300px", gap: "16px", alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          <div style={{ fontSize: "11px", fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: "0.07em" }}>Productos</div>
          <input
            placeholder="🔍 Buscar producto..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            style={{ width: "100%", padding: "9px 12px", borderRadius: "8px", border: "1px solid #e0e0e0", fontSize: "13px", boxSizing: "border-box", backgroundColor: "#fff", outline: "none" }}
          />
          ...
          <div style={{ backgroundColor: "#fff", border: "1px solid #e0e0e0", borderRadius: "10px", overflow: "hidden", maxHeight: "calc(100vh - 260px)", overflowY: "auto" }}>
            {filtered.length === 0 && (
              <div style={{ padding: "32px 16px", textAlign: "center", color: "#bbb", fontSize: "13px" }}>Sin resultados</div>
            )}
            {filtered.map(p => {
              const enPedido = p.tallas?.some((t: any) => (draftItems[t.id] || 0) > 0)
              ...
              return (
                ...
                >
                  ...
                  {enPedido && (
                    <span style={{ flexShrink: 0, width: "8px", height: "8px", borderRadius: "50%", backgroundColor: "#2563eb", display: "inline-block" }} />
                  )}
                </div>
              )
            })}
          ...
          {!selectedProduct ? (
            ...
            <div style={{ backgroundColor: "#fff", border: "1px solid #e0e0e0", borderRadius: "12px", overflow: "hidden" }}>
              <div style={{ display: "flex", gap: "0", borderBottom: "1px solid #f0f0f0" }}>
                <div style={{ width: "160px", flexShrink: 0, backgroundColor: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center", minHeight: "160px" }}>
                  {selectedImageUrl
                    ? <img src={selectedImageUrl} style={{ width: "160px", height: "160px", objectFit: "cover", display: "block" }} />
                ...
                <div style={{ flex: 1, padding: "20px 22px", display: "flex", flexDirection: "column", justifyContent: "center", gap: "6px" }}>
                  <div style={{ fontSize: "17px", fontWeight: 700, color: "#111", lineHeight: 1.3 }}>{selectedProduct.nombre}</div>
                  {selectedProduct.color && (
                    <div style={{ display: "inline-flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "#6b7280" }}>
                      <span style={{ width: "8px", height: "8px", borderRadius: "50%", backgroundColor: "#9ca3af", display: "inline-block" }} />
                      {selectedProduct.color}
                    </div>
                  )}
                  {selectedProduct.codigo && <div style={{ fontSize: "12px", color: "#9ca3af", fontFamily: "monospace" }}>{selectedProduct.codigo}</div>}
                  {selectedProduct.departamento && (
                    <div style={{ marginTop: "4px", display: "inline-block", fontSize: "11px", fontWeight: 600, color: "#2563eb", backgroundColor: "#eff6ff", padding: "3px 8px", borderRadius: "20px", alignSelf: "flex-start" }}>
                      {selectedProduct.departamento}
                    </div>
                  )}
                </div>
              ...
              <div style={{ padding: "20px 22px" }}>
                <div style={{ fontSize: "11px", fontWeight: 700, color: "#aaa", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "14px" }}>Unidades a pedir</div>
                    ...
                    return (
                      <div key={t.id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 14px", borderRadius: "8px", backgroundColor: hasQty ? "#eff6ff" : "#fafafa", border: `1px solid ${hasQty ? "#bfdbfe" : "#f0f0f0"}`, transition: "background-color 0.15s, border-color 0.15s" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "0" }}>
                          <button onClick={() => setCantidad(t.id, Math.max(0, qty - 1))} style={{ width: "32px", height: "32px", borderRadius: "8px 0 0 8px", border: "1px solid #e0e0e0", borderRight: "none", backgroundColor: "#fff", color: "#374151", fontSize: "16px", fontWeight: 600, cursor: "pointer", lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                          <input
                            type="number" min="0"
                            value={qty === 0 ? "" : qty}
                            placeholder="0"
                            onChange={e => setCantidad(t.id, Math.max(0, Number(e.target.value) || 0))}
                            style={{ width: "52px", height: "32px", border: "1px solid #e0e0e0", fontSize: "14px", fontWeight: hasQty ? 700 : 400, textAlign: "center", color: hasQty ? "#1d4ed8" : "#374151", backgroundColor: hasQty ? "#eff6ff" : "#fff", outline: "none", MozAppearance: "textfield" as any }}
                          />
                          <button onClick={() => setCantidad(t.id, qty + 1)} style={{ width: "32px", height: "32px", borderRadius: "0 8px 8px 0", border: "1px solid #e0e0e0", borderLeft: "none", backgroundColor: "#fff", color: "#374151", fontSize: "16px", fontWeight: 600, cursor: "pointer", lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
                        </div>
                      </div>
                    )
                  })}
        ...
        {/* COLUMNA 3: RESUMEN */}
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          <div style={{ fontSize: "11px", fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: "0.07em" }}>Resumen del pedido</div>
          <div style={{ backgroundColor: "#fff", border: "1px solid #e0e0e0", borderRadius: "10px", overflow: "hidden", maxHeight: "calc(100vh - 230px)", overflowY: "auto" }}>
            <div style={{ padding: "14px 16px", borderBottom: "1px solid #f0f0f0" }}>
              <div style={{ fontSize: "11px", fontWeight: 700, color: "#aaa", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "8px" }}>Notas</div>
              <textarea
                value={draftNotas}
                onChange={e => setNotas(e.target.value)}
                placeholder="Proveedor, albarán, condiciones de entrega…"
                rows={4}
                style={{ width: "100%", resize: "vertical", padding: "9px 12px", borderRadius: "7px", border: "1px solid #ddd", fontSize: "13px", boxSizing: "border-box", outline: "none" }}
              />
              <div style={{ fontSize: "11px", color: "#aaa", marginTop: "6px" }}>Se guardan con el borrador y quedan visibles en el historial.</div>
            </div>
            {items.length === 0 ? (
              <div style={{ padding: "40px 20px", textAlign: "center", color: "#bbb", fontSize: "13px" }}>Sin productos en el pedido</div>
            ) : (
              <>
                {items.map(item => {
                  const tallasResumen = [...item.tallas].sort((a, b) => ordenarTallas(a.talla, b.talla))
                  return (
                    <div key={item.producto.id} style={{ padding: "12px 14px", borderBottom: "1px solid #f0f0f0" }}>
                      <div style={{ fontWeight: 600, fontSize: "13px", marginBottom: "8px", color: "#111" }}>
                        ...
                        {item.producto.color && <span style={{ fontWeight: 400, color: "#888", marginLeft: "6px", fontSize: "12px" }}>({item.producto.color})</span>}
                      </div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
                        {tallasResumen.map((t: any) => (
                          <div key={t.tallaId} style={{ display: "flex", alignItems: "center", gap: "5px", padding: "3px 10px", borderRadius: "20px", backgroundColor: "#eff6ff", border: "1px solid #bfdbfe", fontSize: "12px" }}>
                            ...
                            <input
                              ...
                              style={{ width: "36px", padding: "0", border: "none", fontSize: "12px", fontWeight: 600, textAlign: "center", color: "#1d4ed8", backgroundColor: "transparent", outline: "none" }}
                            />
                            <button onClick={() => eliminarTalla(t.tallaId)} style={{ background: "none", border: "none", color: "#93c5fd", cursor: "pointer", fontSize: "11px", lineHeight: 1, padding: "0 0 0 2px" }}>✕</button>
                          </div>
                        ))}
                      </div>
                      <div style={{ fontSize: "11px", fontWeight: 600, color: "#6b7280", marginTop: "8px" }}>Subtotal: {item.total} ud.</div>
                    </div>
                  )
                })}
