...
- **Rendimiento en tablas grandes:** las filas de tabla con muchas celdas se extraen a un componente separado envuelto en `React.memo` (ver `ProductRow` en `CatalogView.tsx`). Pasar solo la porción relevante del estado a cada fila (ej. `salidasMap.get(id)` en lugar del mapa entero) para que `memo` pueda saltarse re-renders. Al actualizar un Map en estado, crear siempre un nuevo objeto interno para la entrada modificada — nunca mutar el Map existente.
