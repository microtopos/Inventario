...
function StepperBtn({ side, onClick }: { side: "left" | "right"; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      style={{
        ...
        fontSize: "18px", fontWeight: 500, lineHeight: 1, cursor: "pointer",
...
export default function OrderHistoryPage({
  ...
  function startEditing(itemId: number, currentEffective: number) {
    ...
    setEditValue(currentEffective)
  }

  async function handleSaveModify(itemId: number) {
    const n = editValue === "" ? null : Number(editValue)
    if (editValue !== "" && (!Number.isFinite(n) || n! < 0)) {
      ...
      grouped.forEach((g: any, idx: number) => {
        ...
        for (const t of tallas) {
          ...
          if (selectedOrder.recibido && t.stock_actual !== undefined) {
            doc.setFontSize(7); doc.setTextColor(...grisClar)
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#f5f5f5", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ backgroundColor: "#fff", borderBottom: "1px solid #e0e0e0", padding: "0 32px", display: "flex", alignItems: "center", height: "48px", gap: "12px" }}>
        ...
        {pendingCount > 0 && (
          <span style={{ backgroundColor: "#fef3c7", color: "#92400e", border: "1px solid #fcd34d", borderRadius: "20px", padding: "2px 10px", fontSize: "12px", fontWeight: 600 }}>
      ...
      <main style={{ maxWidth: "1100px", margin: "0 auto", padding: "28px 24px", display: "flex", gap: "24px", boxSizing: "border-box" }}>
        ...
        <div style={{ width: "320px", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: "10px", alignItems: "center", justifyContent: "space-between", marginBottom: "10px", padding: "10px 12px", backgroundColor: "#fff", border: "1px solid #e0e0e0", borderRadius: "10px" }}>
            <button onClick={() => orderSort.toggleSort("fecha")} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontSize: "12px", fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              Fecha{orderArrow("fecha")}
            </button>
            <button onClick={() => orderSort.toggleSort("recibido")} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontSize: "12px", fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: "0.05em" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {!selectedOrder ? (
            <div style={{ backgroundColor: "#fff", border: "1px solid #e0e0e0", borderRadius: "10px", padding: "60px 40px", textAlign: "center", color: "#aaa", fontSize: "15px" }}>
            ...
            <div style={{ backgroundColor: "#fff", border: "1px solid #e0e0e0", borderRadius: "10px", overflow: "hidden" }}>
              ...
              {/* CABECERA */}
              <div style={{ padding: "20px 24px", borderBottom: "1px solid #f0f0f0", backgroundColor: "#fafafa" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "16px" }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: "17px", color: "#111" }}>Pedido #{selectedOrder.id}</div>
              ...
              <div style={{ padding: "20px 24px" }}>
                ...
                {grouped.length === 0 ? (
                  ...
                  grouped.map((g: any, i: number) => (
                    <div key={i} style={{ marginBottom: "16px", border: "1px solid #f0f0f0", borderRadius: "8px", overflow: "hidden" }}>
                      <div style={{ padding: "10px 16px", backgroundColor: "#f9f9f9", borderBottom: "1px solid #f0f0f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        ...
                        <tbody>
                          {g.tallas.map((t: any, j: number) => {
                            ...
                            if (isEditing) {
                              const currentVal = editValue === "" ? 0 : Number(editValue)
                              return (
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: "16px", flexWrap: "wrap" }}>
                                      ...
                                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                        ...
                                        <div style={{ display: "flex", alignItems: "center" }}>
                                          <StepperBtn side="left" onClick={() => setEditValue(Math.max(0, currentVal - 1))} />
                                          <input
                                            autoFocus
                                            type="number"
                                            min={0}
                                            value={editValue}
                                            onChange={e => setEditValue(e.target.value === "" ? "" : Math.max(0, Number(e.target.value)))}
                                            onKeyDown={e => {
